...
func NewCreateFederationRequest(server string, body CreateFederationJSONRequestBody) (*http.Request, error) {
	...
	buf, err := json.Marshal(body) //Converte body in una sequenza di byte contenente il suo equivalente JSON, buf è un []byte che rappresenta il JSON risultante
	if err != nil {
		return nil, err
	}
	bodyReader = bytes.NewReader(buf) //Converte il []byte in un oggetto byte.Reader che implementa l'interfaccia io.Reader
	//bodyReader è un io:reader che è utilizzato per leggere i dati di buf per poi inviarli in una richiesta HTTP
	return NewCreateFederationRequestWithBody(server, "application/json", bodyReader) //Richiesta HTTP
}

// NewCreateFederationRequestWithBody genere la richiesta (CreateFederation) di federazione con qualsiasti tipo di body
